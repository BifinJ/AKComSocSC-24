export default function AboutCard() {
    ...
    return (
        <div className="w-full lg:w-4/5 p-2 flex flex-col md:flex-row md:gap-0 gap-3 items-center justify-center md:mt-24 lg:mt-32">
            {cards.map((card, index) => (
            <div
                key={index}
                className={`relative flex flex-col gap-4 items-center p-4 md:p-6 w-full md:w-[428px] h-64 xs:h-60 ${
                index === 1
                    ? "bg-[url('/images/about-card-bg.png')] bg-cover bg-center scale-100 md:scale-105 transform transition-transform duration-300 z-30 text-cwhite"
                    : "bg-c5 z-20 text-c1"
            ...
            >
                ...
                src={card.icon}
                alt=""
                className={` w-6 md:w-8 h-6 md:h-8 relative ${index === 1 ? "text-cwhite" : "text-c1"}`}
                />
